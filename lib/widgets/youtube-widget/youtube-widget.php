...
class jouy_Youtube_Widget extends WP_Widget {
    /**
     * @TODO - Rename "jouy-profile-widget" to the name your your widget
     *
     ...
     * @var      string
     */
    protected $widget_slug = 'jouy-youtube-widget';
	/*--------------------------------------------------*/
	/* Constructor
	/*--------------------------------------------------*/
	/**
	...
	public function __construct() {
		...
		parent::__construct(
			...
			__( 'Jouy Youtube Widget', 'jouy' ),
			array(
				'classname'  => $this->get_widget_slug().'-class',
				'description' => __( 'Youtube Gallery Widget - shows your latest videos from YT.', 'jouy' )
			)
		);
	...
	public function widget( $args, $instance ) {
	   // PART 1: Extracting the arguments + getting the values
		...
		if (!empty($channel_id)) {
			if ( false === ( $output = get_transient( 'jouy_youtube_widget' ) ) ) { // transient
				...
				set_transient('jouy_youtube_widget', $output, 30 * MINUTE_IN_SECONDS);
			}
			echo $output;
			
		} else {
			_e('Setup not complete. Please check the widget options.', 'jouy');
		} 
	...
	public function update( $new_instance, $old_instance ) {
		...
		delete_transient('jouy_youtube_widget'); // delete transient
	...
	public function form( $instance ) {
	   // PART 1: Extract the data from the instance variable
		...
		<p><?php _e('Enter your YouTube <a href="https://support.google.com/youtube/answer/3250431" target="_blank">Channel ID</a>.', 'jouy'); ?></p>
		<p><?php _e('For example, the red part below:', 'jouy'); ?></p> <p><?php echo esc_url('https://youtube.com/channel/'); ?><span style="color:red">UCyxZB7SqkRFqij18X1rDYHQ</span></p>
		
		<p>
			<label for="<?php echo $this->get_field_id('channel_id'); ?>"><?php _e('YouTube Channel ID:', 'jouy'); ?>
...
add_action( 'widgets_init', create_function( '', 'register_widget("jouy_Youtube_Widget");' ) );
// Hooks fired when the Widget is activated and deactivated
// TODO: Remember to change 'Widget_Name' to match the class name definition
register_activation_hook( __FILE__, array( 'jouy Youtube Widget', 'activate' ) );
register_deactivation_hook( __FILE__, array( 'jouy Youtube Widget', 'deactivate' ) );
