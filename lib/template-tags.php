...
if ( ! function_exists( 'jouy_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time and author.
	 */
	function jouy_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( '%s', 'post date', 'jouy' ),
			'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
		);

		// $byline = sprintf(
		// 	/* translators: %s: post author. */
		// 	esc_html_x( 'by %s', 'post author', 'jouy' ),
...
if ( ! function_exists( 'jouy_entry_categories' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function jouy_entry_categories() {
		/* translators: used between list items, there is a space after the comma */
		$categories_list = get_the_category_list( esc_html__( ', ', 'jouy' ) );
		if ( $categories_list ) {
			/* translators: 1: list of categories. */
			printf( '<span class="cat-links">in: ' . esc_html__( '%1$s', 'jouy' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}
	}
endif;

if ( ! function_exists( 'jouy_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function jouy_entry_footer() {
		// Share functionality
		jouy_share_buttons();
		...
		if ( 'post' === get_post_type() && is_single() ) {
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'jouy' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged: %1$s', 'jouy' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'jouy' ),
						array(
							'span' => array(
								'class' => array(),
							),
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'jouy' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'jouy_social_media' ) ) :
	/**
	 * Prints social media icons with links set in the customizer
	 */
	function jouy_social_media() {
		?>
...
if ( ! function_exists( 'jouy_related_posts_tags' ) ) :
	/**
	 * Shows related Posts by tags
	 */
	function jouy_related_posts_tags() {
	    global $post;
	    $orig_post = $post;
	    ...
	    if ($tags) {
	    	...
	    	if( $my_query->have_posts() ) {
	    		?>
	    		<div id="related-posts"><h3 class="related-title"><?php echo get_theme_mod('jouy_related_headline', 'You might also enjoy');?></h3>
	    		<?php
...
if ( ! function_exists( 'jouy_related_posts_categories' ) ) :
	/**
	 * Shows related Posts by categories
	 */
	function jouy_related_posts_categories() {
			global $post;
		    $orig_post = $post;
...
if ( ! function_exists( 'jouy_share_buttons' ) ) :
	/**
	 * Shows social share buttons
	 */
	function jouy_share_buttons() {
		?>
...
if ( ! function_exists( 'jouy_custom_query' ) ) :
	/**
	 * Simple custom WP Query Function
	 */
	function jouy_custom_query($args, $template) {
		// Define query args
		$my_args = $args;
